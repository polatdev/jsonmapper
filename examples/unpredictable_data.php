 ...
 * awesome features of JsonMapper's API to work with the data anyway!
 *
 ...
 */
class UnpredictableContainer extends JsonMapper
{
    /**
    ...
    {
        // Tell JsonMapper to give us all of our internal data as an array.
        ...
        // going to affect the actual JsonMapper object's own data. That
        ...
        // unpredictable data). So if the user serializes this JsonMapper
 ...
 * detached from the core JsonMapper storage, we will need another solution!
 *
...
{
    /**
     * Syncs the user list cache with the JsonMapper core storage.
     *
    ...
    {
        // If no internal cache exists yet, get its value from JsonMapper.
        if ($this->_userList === null) {
            $this->getUserList(); // Builds our "_userList" variable. Throws.
        }

        // Now, we need to create a new, internal JsonMapper data array for
        ...
        foreach ($this->_userList as $k => $v) {
            $newObjectData[$k] = is_object($v) && $v instanceof JsonMapper
                ? $v->asArray() // Throws.
                : $v; // Is already a valid value.
        }

        // Now give the new object data to JsonMapper, which ensures that it
        ...
        array $userList)
    {
        ...
        // Now sync our internal cache with the JsonMapper object...! :-)
...
/*
 * Now let's look at the internal JsonMapper data storage...
 *
...
/*
 * Now let's SYNC our cache back to the internal JsonMapper data storage!
 *
 * And voila...!
 */
...
/*
 * Now let's look at the contents of our cache AND the JsonMapper data!
 *
 ...
 */
class UnpredictableMixtureContainer extends JsonMapper
{
     ...
     *
     * NOTE: This overrides the normal, automatic JsonMapper getter! By
     ...
     *
     * NOTE: This overrides the normal, automatic JsonMapper setter! By
     * naming our function identically, we override its behavior!
     *
        ...
        array $employees)
    {
        ...
        foreach ($this->_employees as $k => $v) {
            $newInnerValue[$k] = is_object($v) && $v instanceof JsonMapper
                ? $v->asArray() // Throws.
                : $v; // Is already a valid value.
        }
 ...
 * JsonMapper setter, since that property is completely defined. And since
 * the "manager" object is owned by the JsonMapper core, we're able to chain
 ...
 * container", then we CAN just tell JsonMapper to map that property to a
 ...
 * the new data back into the core `JsonMapper` container. Most people will
 * not need to use the setter. It's just provided as an extra feature.
 *
 */
class CoreUnpredictableContainer extends JsonMapper
{
 ...
 * override their `$_type` property to any other valid JsonMapper class.
 */
 ...
 */
class UnpredictableMixtureContainerTwo extends JsonMapper
{
    const JSON_PROPERTY_MAP = [
 ...
 *
 * The container itself takes care of updating the JsonMapper data storage!
 */
...
/*
 * Now finish by looking at all of the data again, via the JsonMapper core
 ...
 * manipulate the unpredictable data via JsonMapper's automatic functions!

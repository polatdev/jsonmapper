...
/**
 * Describes the behavior of a JsonMapper property.
 *
...
class PropertyDefinition
{
        ...
        $required = false)
    {
        ...
        // to be a reachable JsonMapper-based class.
        $this->isObjectType = true;

        // Check if they've used the special shortcut for the core class.
        if ($this->propType === 'JsonMapper') {
            $this->propType = '\\' . JsonMapper::class;
        }
        ...
        // for use as a target type. It must be based on JsonMapper.
        try {
            ...
            // The target class or its parents MUST inherit JsonMapper,
            ...
            // plain "JsonMapper" objects. It's a very bad idea, since
            // they don't get any property definitions, and therefore their
            // object would be unreliable. But that's the user's choice.
            if ($fullClassPath !== JsonMapper::class
                && !$reflector->isSubClassOf('\\' . JsonMapper::class)) {
                throw new BadPropertyDefinitionException(sprintf(
                    'Class "\\%s" must inherit from JsonMapper.',
                    $fullClassPath
                ));
            }

            // Alright, the class path has been fully resolved, validated
            // to be a JsonMapper, and normalized into its correct name.
            // ... Rock on! ;-)

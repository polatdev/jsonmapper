...
namespace Polat\JsonMapper\Property;

use Polat\JsonMapper\Exception\BadPropertyDefinitionException;
use Polat\JsonMapper\Exception\BadPropertyMapException;
use Polat\JsonMapper\Exception\CircularPropertyMapException;
...
class PropertyMapCompiler
{
        ...
        $solveClassName)
    {
        ...
        // valid class-path) JsonMapper classes. However, the actual CLASS
    ...
    private function _processCurrentClass()
    {
        ...
        try {
            ...
            if (!$foundConstant) {
                ...
                // since the class inherits from JsonMapper.
        ...
        if (isset($this->_propertyMapCache->classMaps[$this->_currentClassInfo['className']])) {
            ...
            // FIRST.
            $this->_currentClassPropertyMap = $this->_propertyMapCache->classMaps[$this->_currentClassInfo['className']];
        } else {
            ...
            $this->_propertyMapCache->classMaps[$this->_currentClassInfo['className']] = $this->_currentClassPropertyMap;
        ...
        if (is_string($propName) && is_string($propDefStr)) {
            ...
            if (is_int($propName) && is_string($propDefStr)) {
                ...
                if (class_exists($strictImportClassName)
                    && is_subclass_of($strictImportClassName, '\\' . JsonMapper::class)) {
                    ...
                    // of JsonMapper (we don't bother allowing to import
                    // JsonMapper itself, since it is the lowest possible
        ...
        $required)
    {
        try {
            ...
            if (!isset($this->_currentClassPropertyMap[$propName])
                ...
                // it's pointing at a JsonMapper if it's an object. But we
        ...
        $strictImportClassName)
    {
        ...
        // example, "A extends JsonMapper, imports B", "B extends
        // JsonMapper, imports A". Imagine that you construct "A", which
        // locks "A" during its compilation process. The parent "JsonMapper"
        ...
        // inheritance hierarchy and sees nothing locked (JsonMapper is
     ...
     *                          that it is a valid `JsonMapper` class.
     *
     * @throws BadPropertyDefinitionException
     * @throws BadPropertyMapException
     * @throws CircularPropertyMapException

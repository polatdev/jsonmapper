...
namespace Polat\JsonMapper;

use Polat\JsonMapper\Exception\JsonMapperException;
use Polat\JsonMapper\Exception\LazySerializationException;
use Polat\JsonMapper\Exception\LazyUserException;
use Polat\JsonMapper\Exception\LazyUserOptionException;
use Polat\JsonMapper\Exception\MagicTranslationException;
use Polat\JsonMapper\Exception\RequiredPropertyException;
use Polat\JsonMapper\Export\ClassAnalysis;
use Polat\JsonMapper\Export\PropertyDescription;
use Polat\JsonMapper\Magic\FunctionTranslation;
use Polat\JsonMapper\Property\PropertyDefinition;
use Polat\JsonMapper\Property\PropertyMapCache;
use Polat\JsonMapper\Property\PropertyMapCompiler;
use Polat\JsonMapper\Property\UndefinedProperty;
use Polat\JsonMapper\Property\ValueConverter;
use stdClass;
 ...
 *    your `JsonMapper`-based classes so that their virtual properties and
 ...
 *    classes, then simply create a subclass of `JsonMapper` and configure
 ...
 *    And everything we throw is _always_ based on `JsonMapperException`,
...
class JsonMapper
{
     ...
     * of `JsonMapper` which disables this option, and then you simply
     ...
     * @var bool
     *
     * @see JsonMapper::__get() More details about virtual properties.
     */
     ...
     * @var bool
     *
     * @see JsonMapper::__call() More details about virtual functions.
     */
     ...
     *
     *   ```php
     *   class MyUncachedJsonMapper extends JsonMapper
     *   {
     *       const USE_MAGIC_LOOKUP_CACHE = false;
     *   }
     *   ```
     *
     * With the above code, `MyUncachedJsonMapper` (and anything extending
     ...
     * classes MUST inherit from `JsonMapper` so they support all necessary
     ...
     *
     * It's also possible to map properties to the core `JsonMapper` object
     ...
     * either `\JsonMapper\JsonMapper`, or as `JsonMapper` (which is
     ...
     * integral part of the `JsonMapper` container, since it guarantees your
     ...
     *     'array_of_generic_objects' => 'JsonMapper[]',
     * ];
     * ```
     *
     ...
     * @var array
     *
     * @see JsonMapper::printPropertyDescriptions() Easily looking at and
     *                                                  debugging your final
     *                                                  class property map.
     * @see JsonMapper::printJson()                 Looking at the JSON data
     *                                                  contents of the current
     *                                                  object instance.
     * @see JsonMapper::exportClassAnalysis()       Looking for problems
     *                                                  with your class map.
     ...
     *
     * If property wasn't in JSON data we throw exception about it
     *
     * Example of importing one or more maps from other classes:
     *
     * ```php
     ...
     *
     * Globally shared across all instances of `JsonMapper` classes. That
     ...
     * @var array
     *
     * @see JsonMapper::clearGlobalMagicLookupCache()
     */
     ...
     *
     * Globally shared across all instances of `JsonMapper` classes. Which
     ...
     *
     * @see JsonMapper::clearGlobalPropertyMapCache()
     */
     ...
     *
     * @throws JsonMapperException If the class hierarchy contains any
     ...
     * @see JsonMapper::assignObjectData()
     */
     ...
     *
     *  1. As explained above, but worth repeating again: Your `JsonMapper`
     ...
     *  `JsonMapper` constructor guarantees in its listed exceptions.
     *
     *  (That's also the reason why all built-in functions except `_init()` are
     *  marked `final`: So that subclasses cannot break the `JsonMapper`
     *  API/interface contract. If something IS a `JsonMapper`, it MUST
     *  behave as a `JsonMapper`, and the `final` functions guarantee that!)
     *
     *  3. If your class extends from `JsonMapper`, then there's no need to
    ...
    protected function _init()
    {
        ...
        // direct parent is JsonMapper, since we do nothing in our _init().
     ...
     *                               natural order.
     *
     * @throws JsonMapperException If any properties cannot be described.
     *                                 But that should never be able to happen.
     *
     * @see JsonMapper::printPropertyDescriptions()
     */
        ...
        $includeUndefined = false)
    {
        if (!is_bool($allowRelativeTypes) || !is_bool($includeUndefined)) {
            throw new JsonMapperException('The function arguments must be booleans.');
        }
     ...
     *                                 But that should never be able to happen.
     *
     * @see JsonMapper::exportPropertyDescriptions()
     * @see JsonMapper::printJson()
     */
     ...
     *
     * @throws JsonMapperException If there are any conversion problems.
     *
     * @see JsonMapper::asStdClass()
     * @see JsonMapper::asArray()
     * @see JsonMapper::asJson()
     * @see JsonMapper::printJson()
     */
        ...
        $objectRepresentation = 'array')
    {
        ...
        array_walk_recursive($copy->_objectData, function (&$value, $key) use ($objectRepresentation) {
            ...
            if (is_object($value)) {
                // Verify that the object is an instance of JsonMapper.
                // NOTE: It SHOULDN'T be able to be anything else, since the
                // object analysis above has already verified that any "mixed"
                // (non-JsonMapper) properties only contain basic PHP types.
                // But this check is a cheap safeguard against FUTURE bugs.
                if (!$value instanceof self) {
                    throw new JsonMapperException(sprintf(
                        'Unable to convert data to %s: Unexpected "%s" object in property/key "%s", but we expected an instance of a JsonMapper object.',
                        $objectRepresentation,
     ...
     * @throws JsonMapperException If there are any conversion problems.
     *
     * @see JsonMapper::exportObjectDataCopy()
     * @see JsonMapper::asArray()
     * @see JsonMapper::asJson()
     * @see JsonMapper::printJson()
     */
     ...
     * @see JsonMapper::asArray()
     * @see JsonMapper::printJson()
     * @see JsonMapper::jsonSerialize()        The native json_encode()
     *                                             serializer instead.
     */
        ...
        $depth = 512)
    {
        if (!is_int($options) || !is_int($depth)) {
            throw new JsonMapperException('Invalid non-integer function argument.');
        }
     ...
     * by PHP whenever you attempt to `json_encode()` a `JsonMapper` object
     ...
     * `JsonMapper`-based objects), _without_ needing to manually fiddle
     ...
     * of whether they're pure PHP types or nested `JsonMapper` objects.
     *
    ...
    final public function jsonSerialize()
    {
        ...
        // input ("mixed"/undefined data NOT mapped to `JsonMapper` classes)
        ...
        // to "JsonMapper", which will ensure that they'll be preserved as
     ...
     *
     * If a recursive scan is requested, then all sub-objects (`JsonMapper`
        ...
        $recursiveScan = true)
    {
        ...
        foreach ($this->_objectData as $propName => $value) {
            ...
            try {
                ...
                // illegal data values (such as Resources or non-JsonMapper
                ...
                if ($recursiveScan && $value !== null && $propDef->isObjectType) {
                    ...
                    // JsonMapper class. Also, since the $value is not NULL
     ...
     * @return bool
     *
     * @see JsonMapper::_hasPropertyDefinition()
     * @see JsonMapper::_hasPropertyDefinitionOrData()
     */
     ...
     * @throws JsonMapperException If property isn't defined in the class
     ...
     *
     * @throws JsonMapperException If the value can't be turned into its
     ...
     * @throws JsonMapperException If the new value isn't legal for that
     ...
     * @throws JsonMapperException   If the function type or property name is
     ...
     * @see JsonMapper::_hasPropertyDefinitionOrData()
     * @see JsonMapper::_isProperty()
     * @see JsonMapper::_getProperty()
     * @see JsonMapper::_setProperty()
     * @see JsonMapper::_unsetProperty()
     */
     ...
     * nested, internal `JsonMapper` objects as a single, plain data array,
     ...
     * your `JsonMapper` subclasses are supposed to be designed. If they
     ...
     *
     * @return string|array The object's internal data as a string representation.
    ...
    final public function serialize()
    {
        ...
        // NOTE: This efficiently packs all inner, nested JsonMapper objects
     ...
     *                                    unserialized at all.
     * @throws JsonMapperException    If there are any problems creating the

...
class PropertyDescription
{
     ...
     *
     * @throws JsonMapperException If there are any problems with the input.
     */
        ...
        $allowRelativeTypePath = false)
    {
        if (!is_string($ownerClassName) || $ownerClassName === '') {
            throw new JsonMapperException('The owner class name must be a non-empty string value.');
        }
        if (!is_string($propName) || $propName === '') {
            throw new JsonMapperException('The property name must be a non-empty string value.');
        }
        if (!is_bool($allowRelativeTypePath)) {
            throw new JsonMapperException('The allowRelativeTypePath argument must be a boolean.');
        }

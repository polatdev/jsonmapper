...
/**
 * Container for the result of a JsonMapper class analysis.
 *
 * Describes any problems (bad/missing definitions) of a `JsonMapper` class
 ...
 *
 * @see JsonMapper::exportClassAnalysis()
 */
class ClassAnalysis
{
    /**
     ...
     *
     * @throws JsonMapperException If any of the parameters are invalid.
     *
        ...
        $problemMessage)
    {
        if (!is_string($definitionSource) || !is_string($problemMessage)) {
            throw new JsonMapperException('The definitionSource and problemMessage parameters must be strings.');
        }
        if ($problemType !== 'bad_definitions' && $problemType !== 'missing_definitions') {
            throw new JsonMapperException('The problemType parameter must be either "bad_definitions" or "missing_definitions".');
        }
